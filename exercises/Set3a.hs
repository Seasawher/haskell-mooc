...
firstHalf s =
  if even (length s) then
    take (length s `div` 2) s
  else
    take ((length s + 1) `div` 2) s
...
joinToLength i list = [ fst ++ scd | fst <- list, scd <- list, length (fst ++ scd) == i]
...
sumRights [] = 0
sumRights ((Right i) : list) = i + sumRights list
sumRights ((Left b) : list) = sumRights list
...
interpreter' [] x y = []
interpreter' ("up" : list) x y = interpreter' list x (y + 1)
interpreter' ("down" : list) x y = interpreter' list x (y - 1)
interpreter' ("left" : list) x y = interpreter' list (x - 1) y
interpreter' ("right" : list) x y = interpreter' list (x + 1) y
interpreter' ("printX" : list) x y = (show x) : interpreter' list x y
interpreter' ("printY" : list) x y = (show y) : interpreter' list x y
