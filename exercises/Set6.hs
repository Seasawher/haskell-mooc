...
instance Eq RationalNumber where
  RationalNumber p1 q1 == RationalNumber p2 q2 = p1 * q2 - p2 * q1 == 0
...
simplify (RationalNumber p q) =
  let g = gcd p q in
  RationalNumber (div p g) (div q g)
...
instance Num RationalNumber where
  (RationalNumber p1 p2) + (RationalNumber q1 q2) = simplify ( RationalNumber (p1 * q2 + q1 * p2) (p2 * q2))
  (RationalNumber p1 p2) * (RationalNumber q1 q2) = simplify ( RationalNumber (p1 * q1) (p2 * q2) )
  ...
  negate (RationalNumber p q) = RationalNumber (negate p) q
