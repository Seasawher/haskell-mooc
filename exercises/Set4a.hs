...
allEqual [] = True
allEqual [a] = True
allEqual (a : b : as) = (a == b && allEqual (b : as))
...
distinct [] = True
distinct [x] = True
distinct (a : b : as) = a /= b && distinct (a : as) && distinct (b : as)
...
sortByHead = sortBy (comparing head)

longest :: Ord a => [[a]] -> [a]
longest xs = do
  let len_sorted = reverse (sortByLength xs)
  let max_len = length (head len_sorted)
  let filtered = filter (\x -> length x >= max_len) xs
  let sorted = sortByHead filtered
  head sorted
...
incrementKey key = map (\(x, y) -> if x == key then (x, y+1) else (x, y))
...
winner scores player1 player2 = do
  ...
  let score2 = Map.findWithDefault 0 player2 scores
...
freqs xs = helper xs Map.empty
  ...
  helper [] dict = dict
  helper (x : xs) dict = do
    let count = Map.findWithDefault 0 x dict
    helper xs (Map.insert x (count + 1) dict)
  ...
  case Map.lookup from bank of
    ...
    Just from_v ->
      case Map.lookup to bank of
        Nothing -> bank
        Just to_v ->
          (if (amount < 0) || (from_v < amount) then
            bank
          else (do
            let bank1 = Map.insert from (from_v - amount) bank
            let bank2 = Map.insert to (to_v + amount) bank1
            bank2)
          )
